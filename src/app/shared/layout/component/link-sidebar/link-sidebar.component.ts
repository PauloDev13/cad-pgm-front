  ...
  template: `
    @if (link()) {
      ...
      <button
        ...
        class="menu-subitem group flex items-center h-12 w-full transition-colors justify-center
              px-0 cursor-pointer bg-transparent border-none outline-none hover:bg-blue-50 text-left"
        [class]="onOpen() ? 'md:pl-12 md:justify-start' : 'md:px-0 md:justify-center'"
        matTooltip="{{toolTip()}}" matTooltipPosition="right" [matTooltipDisabled]="onOpen()"
        matTooltipClass="tooltip-blue">
