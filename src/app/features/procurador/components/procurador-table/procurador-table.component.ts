      ...
      <div class="overflow-x-auto w-full flex-1 min-h-0">
        <table mat-table [dataSource]="data()" class="w-full min-w-full">
          ...
          <ng-container matColumnDef="nome">
            ...
            <td mat-cell *matCellDef="let item" class="!font-medium !text-sm !px-3 text-gray-800 truncate max-w-[200px] md:max-w-none">
              {{ item.nome }}
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="tipoCertificado">
            <th mat-header-cell *matHeaderCellDef class="!font-semibold text-gray-800 !text-sm !px-3 !w-[1%] whitespace-nowrap text-center">
              Tipo
            </th>
            <td mat-cell *matCellDef="let item" class="!text-sm !px-3 whitespace-nowrap text-center">
              @if (item.tipoCertificado === 'A1') {
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-sky-100 text-sky-800 border border-sky-300">
                  <mat-icon class="!text-[14px] !w-3.5 !h-3.5 mr-1 leading-none">description</mat-icon>
                  A1
                </span>
              } @else if (item.tipoCertificado === 'A3') {
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-300">
          ...
          <ng-container matColumnDef="dataExpedicao">
            <th mat-header-cell *matHeaderCellDef class="hidden md:table-cell !font-semibold text-gray-800 !text-sm !px-3 !w-[1%] whitespace-nowrap">
              Expedição
            </th>
            <td mat-cell *matCellDef="let item" class="hidden md:table-cell !text-sm !px-3 whitespace-nowrap text-gray-600">
              {{ item.dataExpedicao ? (item.dataExpedicao | date:'dd/MM/yyyy HH:mm') : '-' }}
            </td>
          </ng-container>

          <!-- Coluna Data Expiração -->
          <ng-container matColumnDef="dataExpiracao">
            <th mat-header-cell *matHeaderCellDef class="!font-semibold text-gray-800 !text-sm !px-3 !w-[1%] whitespace-nowrap">
              Expiração
            </th>
            <td mat-cell *matCellDef="let item" class="!text-sm !px-3 whitespace-nowrap">
              <div class="flex items-center gap-2">
                <span class="text-gray-700 font-medium">
                  {{ item.dataExpiracao ? (item.dataExpiracao | date:'dd/MM/yyyy HH:mm') : '-' }}
                </span>
          ...
          <ng-container matColumnDef="acoes">
            ...
            <td mat-cell *matCellDef="let item" class="!text-sm !px-2 md:!px-3 text-gray-600 whitespace-nowrap text-right">
              <div class="flex items-center justify-end gap-1 min-w-max">
                ...
                >
                  <mat-icon class="!text-blue-600 transition-transform duration-200 group-hover:!scale-125 !text-[20px]">
                    edit
                  </mat-icon>
                </button>
                <button
...
export class ProcuradorTableComponent {
  ...
  getStatusExpiracao(dataIso: string): ExpiracaoStatus {
    ...
    if (diffDias < 0) {
      return {
        label: 'Expirado',
        cssClass: 'bg-rose-50 text-rose-700 border-rose-200',
        icon: 'error_outline'
      };
    }
    ...
    if (diffDias <= 30) {
      return {
        ...
        cssClass: 'bg-amber-50 text-amber-700 border-amber-200',
        icon: 'warning_amber'
      };
    }
    ...
    return {
      label: 'Válido',
      cssClass: 'bg-emerald-50 text-emerald-700 border-emerald-200',
